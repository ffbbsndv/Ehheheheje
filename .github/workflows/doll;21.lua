loadstring(game:HttpGet("https://github.com/ffbbsndv/Moon-Convert/raw/main/Scripts/Other/sendNotification.lua", true))()
loadstring(game:HttpGet("https://github.com/ffbbsndv/Moon-Convert/raw/main/Scripts/Other/AlignCharacter.lua", true))()
...
if Character and Character:FindFirstChild("LavanderHair") then
    
    local LavanderHair = Character:FindFirstChild("RARM")
    local KateHair = Character:FindFirstChild("LARM")
    local PalHair = Character:FindFirstChild("Accessory (RArm)")
    local GreenHeadband = Character:FindFirstChild("Accessory (LArm)") 
    local PinkHair = Character:FindFirstChild("Anime Blush Head")
    local RobloxClassicRed = Character:FindFirstChild("Black")
    
    local LHWeld = LavanderHair and LavanderHair.Handle and LavanderHair.Handle:FindFirstChild("AccessoryWeld")
    if LHWeld then LHWeld:Destroy() end
    
    local KHWeld = KateHair and KateHair.Handle and KateHair.Handle:FindFirstChild("AccessoryWeld")
    if KHWeld then KHWeld:Destroy() end
    
    local PHWeld = PalHair and PalHair.Handle and PalHair.Handle:FindFirstChild("AccessoryWeld")
    if PHWeld then PHWeld:Destroy() end
    
    local GHBWeld = GreenHeadband and GreenHeadband.Handle and GreenHeadband.Handle:FindFirstChild("AccessoryWeld")
    if GHBWeld then GHBWeld:Destroy() end
    
    local PiHWeld = PinkHair and PinkHair.Handle and PinkHair.Handle:FindFirstChild("AccessoryWeld")
    if PiHWeld then PiHWeld:Destroy() end
    
    local RCRWeld = RobloxClassicRed and RobloxClassicRed.Handle and RobloxClassicRed.Handle:FindFirstChild("AccessoryWeld")
    if RCRWeld then RCRWeld:Destroy() end

    if LavanderHair and LavanderHair.Handle then
        AlignCharacter(LavanderHair.Handle, riggy["Right Arm"], Vector3.new(0, 0, 0), Vector3.new(90, 0, 0))
    end
    
    if KateHair and KateHair.Handle then
        AlignCharacter(KateHair.Handle, riggy["Left Arm"], Vector3.new(0, 0, 0), Vector3.new(90, 0, 0))
    end
    
    if PalHair and PalHair.Handle then
        AlignCharacter(PalHair.Handle, riggy["Right Leg"], Vector3.new(0, 0, 0), Vector3.new(90, 0, 0))
    end
    
    if GreenHeadband and GreenHeadband.Handle then
        AlignCharacter(GreenHeadband.Handle, riggy["Left Leg"], Vector3.new(0, 0, 0), Vector3.new(90, 0, 0))
    end
    
    if PinkHair and PinkHair.Handle then
        AlignCharacter(PinkHair.Handle, riggy.Head, Vector3.new(0, 0, 0), Vector3.new(90, 0, 0)) 
    end
    
    if RobloxClassicRed and RobloxClassicRed.Handle then
        AlignCharacter(RobloxClassicRed.Handle, riggy.Torso, Vector3.new(0, 0, 0), Vector3.new(90, 0, 0)) 
    end
